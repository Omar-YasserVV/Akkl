...
import type { ReservationDetails, ReservationDiningZone, ReservationPartySize, ReservationResponse } from "@/types/reservation";
...
const partySizes = [1, 2, 3, 4, 5, 6, 7, 8] as const;
const diningZones: ReservationDiningZone[] = ["indoor", "outdoor"];
...
function parsePartySize(value: string | undefined): ReservationPartySize {
  ...
  return partySizes.includes(parsed as ReservationPartySize)
    ? (parsed as ReservationPartySize)
    : 2;
}

function parseDiningZone(value: string | undefined): ReservationDiningZone {
  return diningZones.includes(value as ReservationDiningZone)
    ? (value as ReservationDiningZone)
    : "indoor";
}

function parseRouteParams(params: Record<string, string | string[] | undefined>): ReservationDetails {
  return {
    branchId: firstParam(params.branchId) ?? "",
    branchName: firstParam(params.branchName) ?? "Downtown Branch",
    restaurantName: firstParam(params.restaurantName) ?? "Akkl",
    diningZone: parseDiningZone(firstParam(params.diningZone)),
    date: firstParam(params.date) ?? "",
    time: firstParam(params.time) ?? "",
    partySize: parsePartySize(firstParam(params.partySize)),
    specialRequest: firstParam(params.specialRequest) ?? "",
  };
}
...
export default function ReservationConfirmationScreen() {
  ...
  const details = useMemo<ReservationDetails>(() => parseRouteParams(params), [params]);

  useEffect(() => {
    let mounted = true;

    confirmReservation(details)
      .then((response) => {
        if (mounted) setReservation(response);
      })
      .finally(() => {
    ...
    <View className="flex-1 bg-[#F7F8FA]">
      ...
      >
        ...
        <View className="items-center">
          ...
          <Text className="mt-3 max-w-[260px] text-center text-[14px] font-semibold leading-5 text-[#5A6372]">
            {`Your reservation request for ${details.branchName} has been sent to our host for review. We'll notify you as soon as it's confirmed.`}
          </Text>
        </View>
        ...
        <View className="mt-8 rounded-[12px] border border-[#E2E8F0] bg-white p-4">
          ...
          <DetailRow icon="people-outline" label="Guests" value={`${details.partySize} People`} />
          <DetailRow icon="restaurant-outline" label="Zone" value={capitalize(details.diningZone)} />
        </View>

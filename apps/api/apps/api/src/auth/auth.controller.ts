...
export class AuthController {
  ...
  async signup(@Body() data: CreateUserDto, @Res() res: Response) {
    const result = await lastValueFrom(this.authService.send('signup', data));
  ...
  async login(@Body() data: LoginDto, @Res() res: Response) {
    const result = await lastValueFrom(this.authService.send('login', data));
    this.setAuthCookies(res, result);
  ...
  async completeGoogleSignup(@Body() data: any, @Res() res: Response) {
    const result = await lastValueFrom(
      this.authService.send('complete-google-signup', data),
    );
    this.setAuthCookies(res, result);
    return res.status(HttpStatus.OK).json({ status: 'success', data: result });
  }
  ...
  async forgotPassword(@Body('email') email: string) {
    return this.authService.send('forgot-password', email);
  }

  @Post('reset-password')
  async resetPassword(@Body() resetDto: any) {
    return this.authService.send('reset-password', resetDto);
  }
}